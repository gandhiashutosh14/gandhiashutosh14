...
|---|---|---|
| **[TRACEWAKE](https://github.com/gandhiashutosh14/tracewake)** | Event-sourced governance for AI agents: a governed agent's decision trace published to a Kafka-compatible log (AutoMQ) as policy-stamped envelopes, a ledger rebuilt from the log alone, and a replay that re-decides recorded tool calls under a changed policy without re-executing anything, classifying each as unchanged, flipped, or needs-evidence. | Python · kafka-python · AutoMQ 1.7.4 + MinIO in CI · **68 tests** · 8 of 8 proof checks, 0 replay mismatches, committed reports |
...
|---|---|---|
| [TRACEWAKE](https://github.com/gandhiashutosh14/tracewake) | When an AI agent has acted and the rules later change, nobody can say which past actions would now be refused; this keeps every decision as durable evidence and replays it under the new rules. | Platform, risk and compliance teams running agents that act on real systems |
